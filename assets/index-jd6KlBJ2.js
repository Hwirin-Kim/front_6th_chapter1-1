...
`,v=e=>{let t=()=>{switch(e){case`MainPage`:return y();case`ProductPage`:return ee();default:return y()}},n=l.state.cartsProductCount;return` <header class="bg-white shadow-sm sticky top-0 z-40">
  ...
  </h1> `,ee=()=>`<div class="flex items-center space-x-3">
...
`,S=(e,t,n)=>t?`
  ...
  `:`
    ...
    `,C=()=>{let e=new URLSearchParams(window.location.search),t=e.get(`category1`)||``,n=e.get(`category2`)||``,{categories:r,isLoading:i}=s.state;console.log(r);let a=Object.keys(r)??[],o=[];t&&r[t]&&(o=Object.keys(r[t])||[]);let c=``;return t&&(c+=`
    ...
    <div class="space-y-2">
      ...
      <div class="flex flex-wrap gap-2">
        ${i?`<div class="text-sm text-gray-500 italic">카테고리 로딩 중...</div>`:t===``?a.map(e=>S(e)).join(``):o.map(e=>S(t,e,n===e)).join(``)}
      </div>
      <!-- 2depth 카테고리 -->
    </div>
  `},w=()=>`
  ...
  `,T=()=>{let{pagination:{limit:e}}=o.state;return`
    <div class="flex items-center gap-2">
...
`,ie=()=>{let{products:e,isLoading:t,pagination:n}=o.state;return`
    <div class="mb-6">
      <div>
        <!-- 상품 개수 정보 -->
        ...
        <div class="grid grid-cols-2 gap-4 mb-6" id="products-grid">
          <!-- 로딩 스켈레톤 -->
          ${t?Array.from({length:4}).map(()=>ne()).join(``):e.map(e=>te(e)).join(``)}
        </div>
        ${t?re():``}
      </div>
    </div>
  `};let O=!1;function k(){let{pagination:e}=o.state;if(!e.hasNext)return;let t=document.documentElement.scrollHeight,n=window.scrollY,r=window.innerHeight;if(n+r>=t-100){if(o.state.isLoading)return;m({...e,page:e.page+1})}}const A=async()=>(O||(window.addEventListener(`scroll`,k),O=!0),_(v(`MainPage`)+b(D(),ie())+x())),j=async e=>{c.setState({isLoading:!0,error:null});try{let t=await f(e);return c.setState({productDetail:{...t},isLoading:!1}),t}catch(e){return console.error(e),c.setState({error:`상품 상세 조회 에러`,isLoading:!1}),null}},M=async e=>{let t={page:1,limit:a,search:``,category1:``,category2:``,sort:`price_asc`,...e};u.setState({isLoading:!0,error:null});try{let n=await d(t);console.log(`fetchProducts`,n),e.page===1||e.page===void 0?u.setState({...n,isLoading:!1}):u.setState({...u.state,...n,products:[...u.state.products,...n.products],isLoading:!1})}catch(e){console.error(e),u.setState({error:`관련상품목록 조회 에러`,isLoading:!1})}},N=e=>{let{category1:t,category2:n}=e,r=`
  ...
  `,r=``;for(let i=0;i<5;i++)r+=i<e?t:n;return`<div class="flex items-center">${r}</div>`},L=e=>{let{description:t,image:n,lprice:r,rating:i,reviewCount:a,stock:o,title:s}=e;return`
  ...
  `},R=e=>{let{productId:t,image:n,title:r,lprice:i}=e;return`
  ...
  `,V=async()=>{let e=W(window.location.pathname).split(`/product/`)[1];e&&c.state.productDetail.productId!==e&&!c.state.isLoading&&j(e),c.state.productDetail.category2&&!u.state.isLoading&&(!u.state.products||u.state.products.length===0)&&M({category2:c.state.productDetail.category2});let{isLoading:t,productDetail:n}=c.state,{products:r}=u.state;return t?P():N(n)+L(n)+B()+(r.length>0?z(r.filter(t=>t.productId!==e)):``)},H=async()=>_(v(`ProductPage`)+b(await V())+x()),U=`/front_6th_chapter1-1`,W=(e=window.location.pathname)=>e.startsWith(U)?e.slice(21)||`/`:e,G=e=>U+e;async function K(){let e=W(window.location.pathname),t=e.match(/^\/product\/(.+)$/);return e===`/`||e===``?await A():t?await H():g()}const q=e=>{window.history.pushState({},``,G(e)),$()};var J=class{setItem(e,t){try{let n=JSON.stringify(t);localStorage.setItem(e,n)}catch(e){console.error(`로컬스토리지에 데이터를 저장하는 중 오류 발생:`,e)}}getItem(e){try{let t=localStorage.getItem(e);return t?JSON.parse(t):null}catch(e){return console.error(`로컬스토리지에서 데이터를 가져오는 중 오류 발생:`,e),null}}removeItem(e){try{localStorage.removeItem(e)}catch(e){console.error(`로컬스토리지에서 데이터를 삭제하는 중 오류 발생:`,e)}}clear(){try{localStorage.clear()}catch(e){console.error(`로컬스토리지를 초기화하는 중 오류 발생:`,e)}}},Y=new J;const ae={CARTS:`carts`};var X=ae;window.toastState={type:null,message:null,timeoutId:null};const Z=(e=`success`,t=`알림`)=>{window.toastState.type=e,window.toastState.message=t,window.toastState.timeoutId&&clearTimeout(window.toastState.timeoutId),window.toastState.timeoutId=setTimeout(()=>{window.toastState.type=null,window.toastState.message=null,$()},2e3),$()},Q=()=>{let e=new URLSearchParams(window.location.search),t=document.getElementById(`limit-select`);t&&(t.value=o.state.pagination.limit+``,t.onchange=t=>{let n=Number(t.target.value);e.set(`limit`,n),window.history.replaceState({},``,`${window.location.pathname}?${e.toString()}`),m({...o.state.pagination,limit:n,page:1})});let n=document.getElementById(`sort-select`);n&&(n.value=o.state.filters.sort+``,n.onchange=t=>{let n=t.target.value;e.set(`sort`,n),window.history.replaceState({},``,`${window.location.pathname}?${e.toString()}`),m({sort:n})});let r=document.getElementById(`search-input`);if(r){let t=e.get(`search`)||``;r.value=t,r.onkeydown=t=>{if(t.key===`Enter`){let n=t.target.value;e.set(`search`,n),window.history.replaceState({},``,`${window.location.pathname}?${e.toString()}`),m({search:n,page:1})}}}let i=document.querySelectorAll(`.product-card`);i&&i.forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.productId;q(`/product/${t}`)})});let a=document.querySelectorAll(`.add-to-cart-btn`);a&&a.forEach(e=>{e.addEventListener(`click`,t=>{t.stopPropagation();let n=e.dataset.productId,r={productId:n,title:e.dataset.productTitle,image:e.dataset.productImage,lprice:e.dataset.productPrice,quantity:1,selected:!1},i=Y.getItem(X.CARTS)||[];console.log(`durlwfdsa`,i);let a=i.findIndex(e=>e.productId===n);a>-1?i[a].quantity+=s:i.push(r),Y.setItem(X.CARTS,i),l.setState({cartsProductCount:i.length}),Z(`success`,`장바구니에 추가되었습니다`)})});let s=1,u=c.state.productDetail.stock,d=document.getElementById(`quantity-decrease`),f=document.getElementById(`quantity-increase`),p=document.getElementById(`quantity-input`);d&&d.addEventListener(`click`,()=>{s>1&&(s--,p.value=s)}),f&&f.addEventListener(`click`,()=>{s<u&&s++,p.value=s}),p&&p.addEventListener(`input`,()=>{let e=parseInt(p.value,10);(isNaN(e)||e<1)&&(e=1),e>u&&(e=u),s=e,p.value=s});let h=document.getElementById(`add-to-cart-btn`);if(h){let{productId:e,image:t,lprice:n,title:r}=c.state.productDetail,i={productId:e,image:t,lprice:n,title:r,quantity:s,selected:!1};h.addEventListener(`click`,()=>{let t=Y.getItem(X.CARTS)||[],n=t.findIndex(t=>t.productId===e);n>-1?t[n].quantity+=s:t.push(i),Y.setItem(X.CARTS,t),l.setState({cartsProductCount:t.length}),Z(`success`,`장바구니에 추가되었습니다`),s=1,p.value=s})}let g=document.querySelectorAll(`.related-product-card`);g&&g.forEach(e=>{e.addEventListener(`click`,()=>{let t=e.dataset.productId;q(`/product/${t}`)})});let _=document.querySelectorAll(`.category1-filter-btn`);_&&_.forEach(t=>{t.addEventListener(`click`,()=>{let n=t.dataset.category1;e.set(`category1`,n),window.history.replaceState({},``,`${window.location.pathname}?${e.toString()}`),m({category1:n,page:1})})});let v=document.querySelectorAll(`.category2-filter-btn`);v&&v.forEach(t=>{t.addEventListener(`click`,()=>{let n=t.dataset.category2;e.set(`category2`,n),window.history.replaceState({},``,`${window.location.pathname}?${e.toString()}`),m({category2:n,page:1})})})},$=async()=>{document.body.querySelector(`#root`).innerHTML=await K()+oe(),Q()},oe=()=>{if(!window.toastState.type)return``;let e=`bg-green-600`;return window.toastState.type===`info`&&(e=`bg-blue-600`),window.toastState.type===`error`&&(e=`bg-red-600`),`
