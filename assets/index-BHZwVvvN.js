...
`,T=e=>{let t=()=>{switch(e){case`MainPage`:return E();case`ProductPage`:return D();default:return E()}},n=l.state.cartsProductCount;return` <header class="bg-white shadow-sm sticky top-0 z-40">
  ...
  </header>`},E=()=>`<h1 class="text-xl font-bold text-gray-900">
    <a href="/" data-link="">쇼핑몰</a>
  </h1> `,D=()=>`<div class="flex items-center space-x-3">
...
`,A=(e,t,n)=>t?`
  ...
  `:`
    ...
    `,ee=()=>{let e=new URLSearchParams(window.location.search),t=e.get(`category1`)||``,n=e.get(`category2`)||``,{categories:r,isLoading:i}=s.state;console.log(r);let a=Object.keys(r)??[],o=[];t&&r[t]&&(o=Object.keys(r[t])||[]);let c=``;return t&&(c+=`
    ...
    <div class="space-y-2">
      ...
      <div class="flex flex-wrap gap-2">
        ${i?`<div class="text-sm text-gray-500 italic">카테고리 로딩 중...</div>`:t===``?a.map(e=>A(e)).join(``):o.map(e=>A(t,e,n===e)).join(``)}
      </div>
      <!-- 2depth 카테고리 -->
    </div>
  `},j=()=>`
  ...
  `,M=()=>{let{pagination:{limit:e}}=o.state;return`
    <div class="flex items-center gap-2">
...
`,R=()=>{let{products:e,isLoading:t,pagination:n}=o.state;return`
    <div class="mb-6">
      <div>
        <!-- 상품 개수 정보 -->
        ...
        <div class="grid grid-cols-2 gap-4 mb-6" id="products-grid">
          <!-- 로딩 스켈레톤 -->
          ${t?Array.from({length:4}).map(()=>I()).join(``):e.map(e=>F(e)).join(``)}
        </div>
        ${t?L():``}
      </div>
    </div>
  `};let z=!1;function B(){let{pagination:e}=o.state;if(!e.hasNext)return;let t=document.documentElement.scrollHeight,n=window.scrollY,r=window.innerHeight;if(n+r>=t-100){if(o.state.isLoading)return;m({...e,page:e.page+1})}}const V=async()=>(z||(window.addEventListener(`scroll`,B),z=!0),w(T(`MainPage`)+O(P(),R())+k())),H=async e=>{c.setState({isLoading:!0,error:null});try{let t=await f(e);return c.setState({productDetail:{...t},isLoading:!1}),t}catch(e){return console.error(e),c.setState({error:`상품 상세 조회 에러`,isLoading:!1}),null}},U=async e=>{let t={page:1,limit:a,search:``,category1:``,category2:``,sort:`price_asc`,...e};u.setState({isLoading:!0,error:null});try{let n=await d(t);console.log(`fetchProducts`,n),e.page===1||e.page===void 0?u.setState({...n,isLoading:!1}):u.setState({...u.state,...n,products:[...u.state.products,...n.products],isLoading:!1})}catch(e){console.error(e),u.setState({error:`관련상품목록 조회 에러`,isLoading:!1})}},W=e=>{let{category1:t,category2:n}=e,r=`
  ...
  `,r=``;for(let i=0;i<5;i++)r+=i<e?t:n;return`<div class="flex items-center">${r}</div>`},J=e=>{let{description:t,image:n,lprice:r,rating:i,reviewCount:a,stock:o,title:s}=e;return`
  ...
  `},Y=e=>{let{productId:t,image:n,title:r,lprice:i}=e;return`
  ...
  `,Q=async()=>{let e=window.location.pathname.split(`/product/`)[1];e&&c.state.productDetail.productId!==e&&!c.state.isLoading&&H(e),c.state.productDetail.category2&&!u.state.isLoading&&(!u.state.products||u.state.products.length===0)&&U({category2:c.state.productDetail.category2});let{isLoading:t,productDetail:n}=c.state,{products:r}=u.state;return t?G():W(n)+J(n)+Z()+(r.length>0?X(r.filter(t=>t.productId!==e)):``)},te=async()=>w(T(`ProductPage`)+O(await Q())+k());async function ne(){let e=window.location.pathname,t=e.match(/^\/product\/(.+)$/);return e===`/`||e===``?await V():t?await te():C()}const $=async()=>{document.body.querySelector(`#root`).innerHTML=await ne()+re(),S()},re=()=>{if(!window.toastState.type)return``;let e=`bg-green-600`;return window.toastState.type===`info`&&(e=`bg-blue-600`),window.toastState.type===`error`&&(e=`bg-red-600`),`
  ...
  `},ie=()=>r(async()=>{let{worker:e}=await import(`./browser-CcyfQrG1.js`);return{worker:e}},[]).then(({worker:e})=>e.start({onUnhandledRequest:`bypass`}));window.addEventListener(`popstate`,$);async function ae(){let e=new URLSearchParams(window.location.search),t=e.get(`sort`)||`price_asc`,n=e.get(`search`)||``,r=e.get(`limit`)||a,i={sort:t,search:n,limit:r};m(i),h(),o.subscribe($),s.subscribe($),c.subscribe($),l.subscribe($),u.subscribe($),$()}ie().then(ae);